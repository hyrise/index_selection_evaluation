...
class TestSelectionUtilsGenerator(unittest.TestCase):
    ...
    def test_indexes_by_table(self):
        ...
        expected = {self.table_a: [self.index_0, self.index_2], self.table_b: [self.index_1]}
    ...
    def test_get_utilized_indexes(self):
        class CostEvaluationMock():
            ...
            def calculate_cost(_, workload, indexes):
                assert len(workload.queries) == 1, "get_utilized_indexes' calculate_cost_mock should not be called with workloads that contain more than one query"
                assert indexes == [], "get_utilized_indexes' calculate_cost_mock should not be called with indexes"
        ...
        query_1 = Query(1, "SELECT * FROM tablea as a, tableb as b WHERE a.col0 = 4 AND a.col1 = 17 AND b.col0 = 3;", [self.column_a_0, self.column_a_1, self.column_b_0])
        workload = Workload([query_0, query_1])
        candidates = candidates_per_query(workload, 2, syntactically_relevant_indexes)

        utilized_indexes, query_details = get_utilized_indexes(workload, candidates, CostEvaluationMock())
        ...
        utilized_indexes, query_details = get_utilized_indexes(workload, candidates, CostEvaluationMock(), detailed_query_information=True)
