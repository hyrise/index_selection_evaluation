...
class TestDropHeuristicAlgorithm(unittest.TestCase):
    ...
    def setUp(self):
        ...
        query_1 = Query(1, 'SELECT * FROM TableA WHERE Col0 = 1 AND Col1 = 2 AND Col2 = 3;', self.all_columns)
        self.database_name = 'test_DB'

        self.workload = Workload([query_0, query_1], self.database_name)
        self.algo.workload = self.workload
        self.algo.cost_evaluation.calculate_cost = MagicMock(side_effect=self._calculate_cost_mock)
    ...
    def _calculate_cost_mock(self, workload, remaining_indexes):
        assert isinstance(remaining_indexes, set), 'The cost mock for the drop heuristic should only be called with index sets'
    ...
    def test_calculate_best_indexes_two_fit(self):
        ...
        self.assertCountEqual(indexes, expected_indexes)
        self.algo.cost_evaluation.calculate_cost.assert_any_call(self.workload, set([self.index_0,self.index_1]))
        self.algo.cost_evaluation.calculate_cost.assert_any_call(self.workload, set([self.index_0,self.index_2]))
        self.algo.cost_evaluation.calculate_cost.assert_any_call(self.workload, set([self.index_1,self.index_2]))
    ...
    def test_calculate_best_indexes_one_fits(self):
        ...
        self.algo.cost_evaluation.calculate_cost.assert_any_call(self.workload, set([self.index_0,self.index_2]))
        self.algo.cost_evaluation.calculate_cost.assert_any_call(self.workload, set([self.index_1,self.index_2]))
        self.algo.cost_evaluation.calculate_cost.assert_any_call(self.workload, set([self.index_0]))
        self.algo.cost_evaluation.calculate_cost.assert_any_call(self.workload, set([self.index_2]))
