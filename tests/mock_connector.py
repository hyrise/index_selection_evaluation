...
query_1 = Query(0, 'SELECT * FROM TableA WHERE Col0 = 4;', [column_A_0])
query_2 = Query(1, 'SELECT * FROM TableA WHERE Col0 = 1 AND Col1 = 2 AND Col2 = 3;', [column_A_0, column_A_1, column_A_2])
...
# Calculate relevant indexes for query1 based on potential indexes
relevant_indexes_query1_table_A = CostEvaluation._relevant_indexes(query_1, table_A_potential_indexes)

# Set assumed cost for combinations of relevant indexes
for index_combination_size in range(0, len(relevant_indexes_query1_table_A) + 1):
    for index_combination in itertools.combinations(relevant_indexes_query1_table_A, index_combination_size):
...
relevant_indexes_query2_table_A = CostEvaluation._relevant_indexes(query_2, table_A_potential_indexes)

# Set assumed cost for combinations of relevant indexes
for index_combination_size in range(0, len(relevant_indexes_query2_table_A) + 1):
    for index_combination in itertools.combinations(relevant_indexes_query2_table_A, index_combination_size):
        # Each index with leading Col0, Col1, Col2 lowers the cost to 20
        if any([(column_A_0, column_A_1, column_A_2) == index.columns[:3] for index in index_combination]):
            mock_cache[(query_2, frozenset(index_combination))] = 20
            continue

        # Each index with leading Col0, Col1 lowers the cost to 25
        if any([(column_A_0, column_A_1) == index.columns[:2] for index in index_combination]):
            mock_cache[(query_2, frozenset(index_combination))] = 25
            continue
        # Each index with leading Col0, Col2 lowers the cost to 27
        if any([(column_A_0, column_A_2) == index.columns[:2] for index in index_combination]):
            mock_cache[(query_2, frozenset(index_combination))] = 27
            continue

        # Each index with leading Col0 lowers the cost to 30
        if any([column_A_0 == index.columns[0] for index in index_combination]):
            mock_cache[(query_2, frozenset(index_combination))] = 30
            continue
        # Each index with leading Col1 lowers the cost to 40
        if any([column_A_1 == index.columns[0] for index in index_combination]):
            mock_cache[(query_2, frozenset(index_combination))] = 40
            continue
        # Each index with leading Col2 lowers the cost to 50
        if any([column_A_2 == index.columns[0] for index in index_combination]):
            mock_cache[(query_2, frozenset(index_combination))] = 50
            continue

        # Default cost without any beneficial index
        mock_cache[(query_2, frozenset(index_combination))] = 150

assert mock_cache[(query_2, frozenset({Index([column_A_0, column_A_1, column_A_2])}))] == 20, f'{mock_cache[(query_2, frozenset({Index([column_A_0, column_A_1, column_A_2])}))]} != 20'
