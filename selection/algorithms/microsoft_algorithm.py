...
class MicrosoftAlgorithm(SelectionAlgorithm):
    ...
    def _calculate_best_indexes(self, workload):
        ...
        for current_max_columns_per_index in range(
                1, self.max_columns_per_index + 1):
            candidates = self.select_index_candidates(workload,
                                                      potential_indexes)
            indexes, _ = self.enumerate_combinations(workload, candidates, current_max_columns_per_index == self.max_columns_per_index)
        ...
        pickle.dump(self.index_history, open(f"benchmark_results/microsoft_index_combination_history.pickle", "wb" ))
        return indexes
    ...
    def enumerate_combinations(self, workload, candidate_indexes, last_round=False):
        log_out = f'Start Enumeration\n' \
        ...
        current_indexes, costs = self.enumerate_naive(workload,
                                                      candidate_indexes,
                                                      number_indexes_naive)

        log_out = f'lowest cost (naive): {costs}\n' \
    ...
    def enumerate_greedy(self, workload, current_indexes, current_costs,
                         candidate_indexes, number_indexes, last_round):
        assert current_indexes & candidate_indexes == set(
        ...
        if best_index[0] and best_index[1] < current_costs:
            ...
            if last_round:
                self.index_history.append(best_index[0])

            logging.debug(f'Additional best index found: {best_index}')

            return self.enumerate_greedy(workload, current_indexes,
                                         current_costs, candidate_indexes,
                                         number_indexes, last_round)
        return current_indexes, current_costs
    ...
    def create_multicolumn_indexes(self, workload, indexes):
        ...
        for index in indexes:
            ...
            for column in (set(index.table().columns) & set(
                    workload.indexable_columns())) - set(index.columns):
