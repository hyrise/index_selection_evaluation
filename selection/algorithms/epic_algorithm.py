...
class EPICAlgorithm(SelectionAlgorithm):
    ...
    def _calculate_best_indexes(self, workload):
        ...
        current_cost = self.cost_evaluation.calculate_cost(self.workload, index_combination,
                                                   store_size=True)
        self.initial_cost = current_cost
        # Breaking when no cost improvement
        while True:
            single_attribute_index_candidates = self._get_candidates_within_budget(index_combination_size, single_attribute_index_candidates)
    ...
    def _attach_to_indexes(self, index_combination, candidate, best):
        assert candidate.is_single_column() is True, 'Attach to indexes called with multi column index'
    ...
    def _get_candidates_within_budget(self, index_combination_size, candidates):
        ...
        for candidate in candidates:
            if (candidate.estimated_size is None) or (candidate.estimated_size + index_combination_size <= self.budget):
