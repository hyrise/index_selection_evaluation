...
def parse_file(file_name):
    ...
    with open(file_name, newline='') as csv_file:
        ...
        for i, row in enumerate(result_reader):
            ...
            results.append((memory, algorithm_runtime, sum(query_costs)))
    return results


if __name__ == '__main__':
    if len(sys.argv) == 1:
        for algorithm in ['anytime', 'auto_admin', 'db2advis', 'dexter', 'drop', 'extend', 'no_index', 'relaxation']:
            result = parse_file(f'../benchmark_results/results_{algorithm}_tpch_19_queries.csv')
            for line in result:
